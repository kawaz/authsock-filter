...
**メインコンテキストでのコード修正・ファイル編集は禁止。**
すべての作業はTaskツールでサブエージェントに委譲すること。

```
❌ 禁止: メインコンテキストで直接 Edit/Write ツールを使う
✅ 必須: Task ツールでサブエージェントを起動し、そこで作業させる
```

**理由:**
- メインコンテキストを節約し、長時間のセッションでも作業継続可能
- サブエージェントは並列実行でき、作業時間を大幅短縮
- 詳細な作業ログがメインに残らず、コンテキスト消費を抑制
...
- Todoリストの管理
- サブエージェントの起動と結果収集
- 最終的なテスト実行（cargo test, cargo clippy）
- ユーザーへの進捗報告

### 3. 並列度の制限

- **推奨並列度: 3〜5**
- 10並列など過剰な並列はセッションが不安定になる
- 独立したタスクは同時にTaskツールを複数呼び出す

### 4. 確認不要

- 「続けますか？」「どれからやりますか？」などの確認は不要
- やるべきことは確認なしで進める
...
```
1. REVIEW_RESULTS.md を読み、未完了タスクを把握
2. 優先度順にTodoリスト作成
3. 独立したタスクをサブエージェントで並列起動（3〜5個）
4. 全サブエージェント完了を待機
5. cargo test && cargo clippy で検証
6. 次のタスク群を並列起動
7. 全完了まで繰り返し
```
...
```
// メインコンテキストで直接Edit
Edit: src/agent/server.rs を修正...
Edit: src/cli/commands/run.rs を修正...
```

**✅ 良い例（サブエージェントに委譲）:**
```
// 3つのタスクを並列起動
Task(subagent_type="general-purpose", prompt="H-1修正: src/agent/server.rsのbind()後にchmod 0600を設定。コード修正のみ、テストは不要。")
Task(subagent_type="general-purpose", prompt="H-2修正: src/cli/commands/run.rsでexists()をsymlink_metadata()に置換。シンボリックリンク検出時はエラー。")
Task(subagent_type="general-purpose", prompt="H-3修正: src/utils/socket.rsを新規作成し、ソケット操作のユーティリティ関数を実装。")
```
...
1. まずプロンプト1〜4（コアレビュー）をサブエージェントで並列実行
2. 続いてプロンプト5〜6（オプションレビュー）を並列実行
3. 続いてプロンプト7〜11（UX・ドキュメント・テストレビュー）を並列実行
4. 最後にプロンプト12（総括レビュー）を実行

各レビュー結果を統合し、指摘事項を優先度順にREVIEW_RESULTS.mdに保存。
```

### Phase 2: 修正作業

```
REVIEW_RESULTS.md を読んで、以下の手順で修正を進めてください:

1. 未完了タスクをTodoリストに追加（優先度順）
2. 独立したタスク3〜5個をサブエージェントで並列起動
   - 各サブエージェントには具体的なファイルパスと修正内容を指示
   - テストは実行させない（メインで一括実行）
3. 全サブエージェント完了後、メインで検証
   - cargo fmt --check
   - cargo clippy -- -D warnings
   - cargo test
4. エラーがあれば修正用サブエージェントを起動
5. 次のタスク群を並列起動
6. 全完了まで繰り返し
```
...
# フルレビュー（推奨）
REVIEW_PROMPTS.md のプロンプト1〜11を順次並列実行し、修正まで完了させる
```
